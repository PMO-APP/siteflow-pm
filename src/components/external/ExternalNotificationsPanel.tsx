...
export default function ExternalNotificationsPanel({ onClose }: Props) {
  const { user } = useAuthStore()
  const role = useMembershipStore(state => state.role)
  const { externalProjectId } = useExternalProjectStore()
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel('external-live-notifications')
      .on(
        'postgres_changes',
        {
          event: '*',
  ...
  }, [user?.id, user?.email, role, externalProjectId])
  ...
  async function loadNotifications() {
    ...
    if (cleanEmail) {
      filters.push(`user_email.eq.${cleanEmail}`)
    }
    ...
    if (externalProjectId) {
      query = query.eq('project_id', externalProjectId)
    }
    ...
    switch (type) {
      case 'external_assignment':
        return (
          <ClipboardList
            size={13}
            className="text-[#c49e48] flex-shrink-0"
          />
        )
  ...
  return (
    <div className="card border border-[#c49e48]/20 shadow-2xl animate-in">
        ...
        {loading ? (
          ...
          notifications.map(notification => (
            <button
              ...
              className={`w-full text-left flex items-start gap-3 px-4 py-3 hover:bg-white/[0.02] transition-colors ${
                !notification.is_read ? 'bg-[#c49e48]/[0.03]' : ''
              }`}
            >
      ...
      <div className="px-4 py-2 border-t border-white/[0.06] text-[10px] text-[#6e7d8c] text-center">
        Alerts from your assigned project activity
      </div>
    </div>
  )
}
