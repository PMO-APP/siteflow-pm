    ...
    <div className="space-y-4 rounded-2xl border border-[var(--pmx-border)] bg-[var(--pmx-surface-2)] p-4">
      ...
      {intelligence.nodes.length === 0 ? <div className="pmx-empty-state">No dependency data is available. Add predecessor relationships to the schedule to activate this view.</div> : (
        <div className="grid gap-4 xl:grid-cols-[minmax(0,1.35fr)_minmax(300px,.65fr)]">
          <div className="overflow-x-auto rounded-xl border border-[var(--pmx-border)] bg-[var(--pmx-surface)] p-4 pmx-scrollbar">
            <div className="flex min-w-max items-center gap-2">
              {intelligence.nodes.slice(0, 12).map((node, index) => (
                <div key={node.id} className="flex items-center gap-2">
                  <button type="button" onClick={() => setSelectedId(node.id)} className={selectedId === node.id ? 'w-44 rounded-xl border border-[var(--pmx-primary)] bg-[var(--pmx-primary-soft)] p-3 text-left' : 'w-44 rounded-xl border border-[var(--pmx-border)] bg-[var(--pmx-surface-2)] p-3 text-left hover:border-[var(--pmx-border-strong)]'}>
                  ...
                  {index < Math.min(11, intelligence.nodes.length - 1) ? <ArrowRight size={15} className="text-[var(--pmx-faint)]" /> : null}
                </div>
              ))}
            </div>
          </div>
